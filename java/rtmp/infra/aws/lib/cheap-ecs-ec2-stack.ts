...
const TRAEFIK_STATIC_CONFIG = fs.readFileSync(path.join(TEMPLATES_DIR, 'traefik.yml'), 'utf8').trimEnd();
const TRAEFIK_DYNAMIC_TEMPLATE = fs.readFileSync(path.join(TEMPLATES_DIR, 'traefik-dynamic.yml'), 'utf8').trimEnd();
...
export function createCheapInfra(scope: Construct, config: InfraConfig): CheapInfraRefs {
  ...
  proxyUserData.addCommands(
    ...
    'EOF',
    'cat > /etc/traefik/ecs-base.yml <<\'EOF\'',
    ...
    '  for ip in $PRIVATE_IPS; do echo "          - address: $ip:1935" >> "$TCP_LIST"; done',
    'fi',
    ...
    '  for ip in $PRIVATE_IPS; do',
    '    STREAMS="$(curl -fsS --max-time 6 "http://$ip:8888/prometheus" 2>/dev/null | awk \'/^rtmp_active_streams/ {print int($2)}\')" || true',
    '    if [ -z "$STREAMS" ]; then STREAMS="$HARD_CAP"; fi',
    '    FREE_SLOTS=$((FREE_SLOTS + HARD_CAP - STREAMS))',
    '  done',
    'fi',
    'NOW_EPOCH="$(date +%s)"',
    'LAST_SIGNAL="$(cat "$LATCH_FILE" 2>/dev/null || echo 0)"',
    'case "$LAST_SIGNAL" in ""|*[!0-9]*) LAST_SIGNAL=0;; esac',
    'if [ "$FREE_SLOTS" -lt "$HARD_CAP" ]; then',
    '  if [ $((NOW_EPOCH - LAST_SIGNAL)) -ge "$SCALE_COOLDOWN_SECONDS" ]; then',
    '    echo "$NOW_EPOCH" > "$LATCH_FILE"',
    ...
    '  echo 0 > "$LATCH_FILE"',
    'fi',
    'echo "scale-signal: freeSlots=$FREE_SLOTS notifications=$NOTIFICATIONS lastSignal=$LAST_SIGNAL"',
    'EOF',
